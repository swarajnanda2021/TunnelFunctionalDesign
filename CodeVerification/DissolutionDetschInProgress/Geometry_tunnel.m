...
QPump        = 0.36*2;                                   % Pump output flowrate [m^2/hr]   
...
TestSection    = 12; % Test-section component number
...
rho_b   = 1.27;                 % Density of bubble (Air) [kg/m^3]    
...
%     opts  = odeset('NonNegative',1);     % No negative terms
%     
%     [TLevich,DLevich] = ode45(@(DiaBub,t) Levich(DiaBub,t,Dc(((i-1)*NN + 1):(i*NN)),D_AB,M_a,sigma,PInfty(((i-1)*NN + 1):(i*NN)),TimeTunnel(((i-1)*NN + 1):(i*NN))), [TimeTunnel((i-1)*NN + 1) TimeTunnel(i*NN)], DLoop,opts);
%     opts2=odeset('Events',@eventfcn,'OutputFcn',@odeplot);
% %     [TLevich,DLevich] = ode45(@(DiaBub,t) Levich(DiaBub,t,TimeTunnel(((i-1)*NN + 1):(i*NN)),C_S(((i-1)*NN + 1):(i*NN)),D_AB,M_a,sigma,PInfty(((i-1)*NN + 1):(i*NN)),f), [TimeTunnel((i-1)*NN + 1) TimeTunnel(i*NN)], DLoop,[opts opts2]);
%     
%     
%     DNew = DLevich(end);
%     RNewLevich = [RNewLevich DNew/2];
% 
%     LevichDEvol = [LevichDEvol DLevich'];
%     LevichTEvol = [LevichTEvol TLevich'];
% 
% end
...
% Plotting
figure(9)
semilogx(TSaltyDimEvol,RSaltyDimEvol)
hold on
semilogx(TDimEvol,RDimEvol)
hold off
...
legend('LevichDirtyBubble','EpsteinPlesset')
%% Rise
