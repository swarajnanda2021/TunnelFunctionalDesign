...
close all
clc
%% Investigating the design of the HYKAT
%% Geometry specifications
% Firstly, geometry parameters require to be loaded. An array containing the 
% static pressures and the cross sectional areas are necessary. Also, it must 
% be recognised where the intake and outlet of the Pipe section exist, in order 
% to account it in the flow-rate calculation.
% 
% In this section, we also calculate the volumetric capacity of the tunnel 
% for 2 reasons: We get an idea of how heavy the tunnel will be and how much heat 
% would be required to raise its temperature by 1 degree celcius. The latter being 
% useful in judging whether a heat-exchanger would be necessary.

% Import main tunnel parameters
NN = 1000;
HTs = 1.1284;          % Height of the tunnel test-section      
...
set(gca, 'YTick', 1:21, 'YTickLabel', TunnelParams(1,:));
grid on
set(gca,'xscale','log')
% xtickangle(90)
% subplot(1,2,2)
...
QPump        = 1;                                   % Pump output flowrate [m^2/hr]   
rho          = 1000;                                   % Fluid density [kg/m^3]
SetP         = 1e5;                                   % Tunnel set pressure [Pascals] 
...
elseif solidity == 0
    for i=1:length(FricMain)
        LossMain(i) = FricMain(i)*(0.5 * rho * (QIMain(i)/MainArInlet(i))^2) * 1.5 ;
    end
end
...
elseif solidity == 0
    ...
    grid on
    barh(LossMain)
    title(['Main Tunnel Head loss: ' num2str(sum(LossMain)) ' Pascals.'])
    xlabel('Total pressure loss (Pascals)')
    set(gca, 'YTick', 1:14, 'YTickLabel', TunnelParams(1,:));
    grid on
    set(gca,'xscale','log')
end
...
BernoulliConstant = min(PStatMain) + (SetP-min(PStatMain)) + max(PDynMain); % Bernoulli constant is determined with respect to set pressure at the test-section
TestSectionJetKE  = (0.5 * rho * ((QPump - max(QPipe))/(HTs^2))^2);
% Calculating $C_p$
PLiquidMain      = BernoulliConstant - (PStatMain + PDynMain);
% Plotting Liquid pressure
if solidity > 0
    BernoulliConstantPipe = min(PStatPipe) + (SetP-min(PStatPipe)) + max(PDynPipe);
    PLiquidPipe      = BernoulliConstant - (PStatPipe + PDynPipe);
    figure(7)
    subplot(2,1,1)
    hold on
    plot(CvomponentsMain(1,:),(PStatMain + PLiquidMain))
    grid on
    title('Liquid pressure + Static pressure (Main)')
    xlabel('Length [m]')
    ylabel('P_{liquid}')
    set(gca,'yscale','log')
    subplot(2,1,2)
    plot(ComponentsPipe(1,:),(PStatPipe + PLiquidPipe))
    grid on
    title('Liquid pressure + Static pressure  (Pipe)')
    xlabel('Length [m]')
    ylabel('P_{liquid}')
    set(gca,'yscale','log')
elseif solidity == 0
    figure(7)
    hold on
    plot(ComponentsMain(1,:),(PStatMain + PLiquidMain))
    grid on
    title('Liquid pressure + Static pressure  (Main)')
    xlabel('Length [m]')
    ylabel('P_{liquid}')
...
if solidity == 0
    ...
    H_z     = (SetP - min(PStatMain))/(rho * 9.81);                      % Static pressure above the pump, the pump is working on positive static head conditions
...
% Modify geometry definition to start from test-section
TestSection    = 1; % Test-section component number
for i=1:length(ComponentsMain(:,1))
    ComponentsMain(:,i) = circshift(ComponentsMain(:,i), [0 -(NN*(TestSection-1))]);
end
PStatMain      = circshift(PStatMain, [0 -(NN*(TestSection-1))]);
PDynMain       = circshift(PDynMain, [0 -(NN*(TestSection-1))]);
QTunnel        = circshift(QTunnel, [0 -(NN*(TestSection-1))]);
...
% Tunnel Parameters
r       = 1e-4;               % Initial bubble radius [m]
f       = 0;                      % Degassing factor [C_i/C_S]
PInfty  = ones(size(PStatMain))*SetP;     

% Fluid and Model Parameters
% D_AB    = 2e-9;%82e-9;              % Diffusion constant (Air-Water)[m^2/sec]
D_AB    = 2.6e-9;               % Diffusion constant (Nitrogen-Water)[m^2/sec]
rho_b   = 1.251;%Nitrogen, 1.225 air                 % Density of bubble (Air) [kg/m^3]    
% mu_b    = 1.81e-5;              % Dynamic viscosity of air
mu_b    = 9.25e-8 * 1.251;%1.8e-5;
fluid   = 'Pure';               % Working fluid (Pure/SeaWater)             

if strcmp(fluid,'SeaWater') == 1
    ...
    sigma       = SW_SurfaceTension(temp,'C',Salinity,'ppt') * 1e-3;                        % Correlation taken from the paper 'surface tension of seawater by L.G. Nayar (MIT)
    rho_f       = SW_Density(temp,'C',Salinity,'ppt',1,'bar');   
    mu_f        = SW_Viscosity(temp,'C',Salinity,'ppt');
            
elseif strcmp(fluid,'Pure') == 1
...
%     C_S = ((M_O2 * P_O2_air * K_H_O2) .* PInfty) + (M_N2 * P_N2_air * K_H_N2) .* PInfty;   
    C_S = ((M_N2 * (1639^-1 /101.325)) .* PInfty);  % Henrys constant for nitrogen in water
...
for i=1:length(TunnelParams(1,:))    
    ...
    [TNonDim,RNonDim]   = ode45(@(r_nd,t_nd) epstein_plesset_nondim_2(r_nd,t_nd, XprimeBernoulli(((i-1)*NN + 1):(i*NN)), f , b(((i-1)*NN + 1):(i*NN)), delta), [XprimeBernoulli(((i-1)*NN + 1)) XprimeBernoulli((i*NN))], RNonDimNext,opts);
...
% Solving the Levich Dirty Bubble formulation
d = 2*r;
Dc = C_S.*(1 - f);
DIn = 2*r;
RNewLevich = [];
RNewLevich = [RNewLevich r];
    
for i=1:length(TunnelParams(1,:))
    DLoop = RNewLevich(i)*2;
    opts  = odeset('NonNegative',1);     % No negative terms
    [TLevich,DLevich] = ode45(@(DiaBub,t) Levich(DiaBub,t,Dc(((i-1)*NN + 1):(i*NN)),D_AB,M_a,sigma,PInfty(((i-1)*NN + 1):(i*NN))), [TimeTunnel((i-1)*NN + 1) TimeTunnel(i*NN)], DLoop,opts);
    DNew = DLevich(end);
    RNewLevich = [RNewLevich DNew/2];
end
...
% plot(TNonDimEvolution,RNonDimEvolution)% plot(1:length(TunnelParams(1,:))+1,RNewLevich')
% hold off
title('Behaviour of bubble along the tunnel length')
xlabel('Time (seconds)')
ylabel('Bubble radius (m)')
legend('EpsteinPlesset','LevichDirtyBubble')
%% Rise
