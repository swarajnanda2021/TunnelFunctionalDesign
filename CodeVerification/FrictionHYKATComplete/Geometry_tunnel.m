...
NN = 1000;
HTs = 1.6;          % Height of the tunnel test-section      
...
QPump        = 11.5*1.6*2.8;                                   % Pump output flowrate [m^2/hr]   
...
elseif solidity == 0
    ...
    set(gca, 'YTick', 1:20, 'YTickLabel', TunnelParams(1,:));
    grid on
    set(gca,'xscale','log')
end
...
PInfty  = PStatMain + ones(size(PStatMain))*SetP;     
...
% Solving Epstein-Plesset's model in non-dimensional format
epsilonini = 1;
REvolution = [];
...
for i=1:length(TunnelParams(1,:))
    ...
    [TNonDim,RNonDim]   = ode45(@(r_nd,t_nd) epstein_plesset_nondim_2(r_nd,t_nd, XprimeBernoulli(((i-1)*NN + 1):(i*NN)), f , b(((i-1)*NN + 1):(i*NN)), delta), [XprimeBernoulli(((i-1)*NN + 1)) XprimeBernoulli((i*NN))], RNonDimNext, [opts opts2]);
