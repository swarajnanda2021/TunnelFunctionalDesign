function [ TunnelParams,PipeParams,ComponentsPipe, ComponentsMain,LossMain,TunnelVolume,RSaltyDimEvol,RDimEvol,TSaltyDimEvol,TDimEvol] = tunneldissolution( r,Hmax,Hmin,SepIn,SepOut,CR,Dpump,Lpump,QPUMP,Dpipe,Buffer,lunchboxwidth,CRpipe,theta )
...
h       = (Hmax)  -  (Hmin + (0.5*SepIn)+0.1+SepOut+SepIn+0.1);
lTop    = (11.5 - 4 + 2.13) + (0.5*SepIn + 0.1) + (0.5*sqrt(CR*0.09*4/pi) + 0.1);
lBottom = Lpump + ((0.5*Dpump + 0.1)*sin(pi/4)) + (0.5*sqrt(0.09*CR*4/pi) + 0.1)*(1-cos(pi/4)) + ( (Hmax - (0.5*Dpump + 0.1)*(1-cos(pi/4))) - (Hmin + (0.5*sqrt(0.09*CR*4/pi) + 0.1)*(1+sin(pi/4)))   );
...
for i=1:length(PipeParams(:,1))
    for j=1:length(PipeParams(1,:))
        if (i>2) && (i<10)
            if isnan(str2double(PipeParams(i,j))) == 1
                PipeParams{i,j} = string(eval(char(PipeParams{i,j})));  
            end
        end
    end
end
...
set(gca, 'YTick', 1:17, 'YTickLabel', TunnelParams(1,:));
grid on
set(gca,'xscale','log')
% xtickangle(90)
subplot(1,2,2)
barh(PipeVolume(:,1))
set(gca, 'YTick', 1:7, 'YTickLabel', PipeParams(1,:));
set(gca,'xscale','log')
%xtickangle(90)
...
SetP         = 101325;                                   % Tunnel set pressure [Pascals] 
nu           = 1e-6;                                   % Fluid kinematic viscosity [m^2/sec]


% % Dividing the flow-rates as per pressure drop caused by solidity using the
% % method described in example 6.18 of White.
% 
% 
...
% Applying the function friction_factor.m to estimate theoretical
% frictional losses in the Main Tunnel
QTunnel = QPump;
[ FricMain, QIMain ] = friction_factors( QTunnel, TunnelParams, ComponentsMain, NN, nu, QPump, max(QPipe), solidity,ValveDia);

% Compute pressure drop between pipe inlet and outlet connections
for i=1
    for j=1:length(PipeParams(1,:))
        if (strcmp(PipeParams(i,j),'Intake')==1) 
            PipeInletConnection = str2double(cellstr(PipeParams(2,j)));
        elseif (strcmp(PipeParams(i,j),'Remerge')==1)
            PipeOutletConnection = str2double(cellstr(PipeParams(2,j)));
        end
    end
end
...
%     for i=1:length(FricMain)
%         
%         zeta0Main(i) = FricMain(i) * (HTs^4 /    MainArInlet(i)^2) ; 
%     end
% %     LossMain = sum(zeta0Main) * TestSectionJetKE;
% elseif solidity > 0 % Normalize w.r.t. main test-section jet KE
%     PipeSectionJetKE = 0.5 * rho * (max(QPipe)/min(ComponentsPipe(2,:)))^2;
%     for i=1:length(FricMain)
%         zeta0Main(i) = FricMain(i) * (HTs^4 / MainArInlet(i)^2);
%     end
%     for i=1:length(FricPipe)
%         zeta0Pipe(i) = FricPipe(i) * (HTs^4 / MainArInlet(i)^2);
%     end
...
if solidity > 0
    ...
    set(gca, 'YTick', 1:18, 'YTickLabel', [TunnelParams(1,:) 'Throttle']);
    grid on
    set(gca,'xscale','log')
    ...
    grid on
    set(gca,'xscale','log')
    title(['Pipe Tunnel Head Loss: ' num2str(sum(LossPipe)/(9.81*rho)) ' m.'])
    xlabel('Pressure loss (Pascals)')
elseif solidity == 0
    figure(6)
    grid on
...
if solidity == 0 % Run this loop only when the valve is at zero position
...
Separator    = 12; % Separator component number
